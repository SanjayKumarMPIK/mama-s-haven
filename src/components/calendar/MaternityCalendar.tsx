...
function getWeeklyInsight(week: number): string {
  if (week <= 0 || week > 40) return "Track your pregnancy journey day by day.";
  if (week <= 4) return "Early days — implantation is happening. Rest and take folic acid.";
  if (week <= 8) return "Morning sickness may start. Small, frequent meals can help.";
  if (week <= 12) return "Fatigue is very common this trimester. Prioritize sleep.";
  if (week <= 16) return "Energy may slowly improve. Focus on balanced nutrition.";
  if (week <= 20) return "You may feel baby's first movements soon! Stay active gently.";
  if (week <= 24) return "Focus on iron intake — your blood volume is increasing.";
  if (week <= 28) return "Third trimester begins soon. Practice relaxation techniques.";
  if (week <= 32) return "Baby is gaining weight rapidly. Eat protein-rich foods.";
  if (week <= 36) return "Prepare your hospital bag. Watch for warning signs.";
  return "You're so close! Rest, hydrate, and trust your body.";
}
...
export default function MaternityCalendar() {
  ...
  const weekDay = useMemo(() => {
    ...
    const totalDays = 280;
    const daysLeftCalc = Math.ceil((due.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
    const daysPassed = totalDays - daysLeftCalc;
    const dayInWeek = ((daysPassed - 1) % 7) + 1;
  ...
  function MonthView() {
    ...
    // Monthly stats — maternity only
    const monthEntries = Object.entries(maternityLogs).filter(([d]) => {
      const [y, m] = d.split("-").map(Number);
      return y === year && m === month0 + 1;
    });
    const totalLogged = monthEntries.length;
    const totalSymptoms = monthEntries.reduce(
      (acc, [, e]) => acc + getMaternitySymptomCount(e),
      0
    );
        ...
        <div className="container py-6 space-y-5">
          ...
          {profile.isSetup && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div className={cn("rounded-xl border p-3 flex items-center gap-3", trimesterColor)}>
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-3">
              {mode === "year"
                ? "Overview of all months. Click any date to log maternity data."
                : "Navigate months and click a date to log symptoms, nutrition, and sleep."}
            </p>
          </div>
        </div>
      </div>
