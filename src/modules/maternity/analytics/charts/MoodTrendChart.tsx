...
export default function MoodTrendChart({ data }: Props) {
  try {
    ...
    const pts = data.map((d, i) => ({ x: getX(i), y: getY(d.moodScore) }));
          ...
          {yLabels.map((yl, idx) => {
            ...
            return (
              <g key={yl.l} className="mc-anim" style={{ animation: `mc-fade 400ms ${idx * 80}ms ease-out both` }}>
                ...
                <text x={pL - 12} y={y + 4} textAnchor="end" fontSize="12" fill="#94a3b8" fontWeight="500" fontFamily="system-ui,sans-serif">{yl.l}</text>
              </g>
            );
          })}
