...
function NextDoseCountdown() {
  ...
  return (
    <div className={`rounded-2xl border p-5 shadow-sm transition-all duration-300 ${isDue
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={`w-11 h-11 rounded-xl flex items-center justify-center shadow-sm ${isDue
...
function DoseCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-5 transition-all duration-300 ${isPending
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center ${isPending
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-auto bg-card rounded-t-3xl sm:rounded-3xl border border-border/60 shadow-2xl p-6 animate-scaleIn max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${isEditing
              ? "bg-gradient-to-br from-blue-500 to-indigo-600"
              : "bg-gradient-to-br from-purple-500 to-violet-600"
              }`}>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className={`w-full py-3 rounded-xl font-semibold shadow-md hover:shadow-lg transition-all active:scale-[0.97] disabled:opacity-40 disabled:cursor-not-allowed text-white ${isEditing
...
function MedicineListCard({
  ...
  return (
    <div
      className={`rounded-xl border p-4 transition-all duration-200 ${medicine.isActive
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={onToggle}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${medicine.isActive
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border ${adherenceRate >= 80
          ? "bg-emerald-50 border-emerald-200"
          : adherenceRate >= 50
            ? "bg-amber-50 border-amber-200"
            : "bg-red-50 border-red-200"
          }`}>
          <TrendingUp className={`w-3.5 h-3.5 ${adherenceRate >= 80 ? "text-emerald-600" : adherenceRate >= 50 ? "text-amber-600" : "text-red-600"
            }`} />
          <span className={`text-xs font-bold ${adherenceRate >= 80 ? "text-emerald-700" : adherenceRate >= 50 ? "text-amber-700" : "text-red-700"
...
function CareLogDashboardCard({ icon, badgeIcon, title, description, accentColor, accentBg, onClick }: DashboardCardProps) {
  return (
    <button
      onClick={onClick}
      className="carelog-dashboard-card group"
      style={{ '--card-accent': accentColor, '--card-accent-bg': accentBg } as React.CSSProperties}
    >
      <div className="carelog-card-inner">
        {/* Icon container */}
        <div className="carelog-card-icon-wrap" style={{ background: accentBg }}>
          <div className="carelog-card-icon">
            {icon}
          </div>
          {badgeIcon && (
            <div className="carelog-card-badge" style={{ color: accentColor }}>
              {badgeIcon}
            </div>
          )}
        </div>
        ...
        {/* Content */}
        <div className="carelog-card-content">
          <h3 className="carelog-card-title">{title}</h3>
          <p className="carelog-card-desc">{description}</p>
        </div>

        {/* Arrow */}
        <div className="carelog-card-arrow" style={{ borderColor: accentBg }}>
...
const CARE_TIPS = [
  "Set reminders and never miss your medicines. Small steps today, healthy tomorrow!",
  "Staying hydrated helps your body absorb supplements better. Drink water with your dose!",
  "Take your iron supplements with vitamin C for better absorption.",
  "Regular ANC visits help monitor your baby's growth — stay on schedule!",
  "Keep a log of your symptoms to discuss with your doctor at your next visit.",
];

function TipBanner() {
  const tipIndex = useMemo(() => Math.floor(Math.random() * CARE_TIPS.length), []);
  return (
    <div className="carelog-tip-banner">
      <div className="carelog-tip-icon">
        <Lightbulb className="w-5 h-5" />
      </div>
      <div className="carelog-tip-label">Tip for You</div>
      <p className="carelog-tip-text">{CARE_TIPS[tipIndex]}</p>
      <div className="carelog-tip-arrow">
...
export default function MedicineReminder() {
  ...
  const dashboardCards: { key: Tab; icon: React.ReactNode; badgeIcon?: React.ReactNode; title: string; description: string; accentColor: string; accentBg: string }[] = [
    {
      key: "today",
      icon: <CalendarDays className="w-7 h-7" style={{ color: '#7c5cfc' }} />,
      badgeIcon: <CheckCircle2 className="w-3.5 h-3.5" />,
      title: "Today Medicines",
      description: "View and manage all medicines scheduled for today.",
      accentColor: "#7c5cfc",
      accentBg: "#f0ecff",
    },
    {
      key: "medicines",
      icon: <Pill className="w-7 h-7" style={{ color: '#e74c7a' }} />,
      badgeIcon: <Plus className="w-3.5 h-3.5" />,
      title: "Medicines",
      description: "Add, edit and track all your medicines in one place.",
      accentColor: "#e74c7a",
      accentBg: "#fde8ef",
    },
    {
      key: "history",
      icon: <History className="w-7 h-7" style={{ color: '#10a882' }} />,
      badgeIcon: <Clock className="w-3.5 h-3.5" />,
      title: "History",
      description: "Check your past medicine intake and logs.",
      accentColor: "#10a882",
      accentBg: "#e6f7f2",
    },
    {
      key: "tests",
      icon: <ClipboardList className="w-7 h-7" style={{ color: '#3b8fd9' }} />,
      badgeIcon: <CheckCircle2 className="w-3.5 h-3.5" />,
      title: "Tests & Scans",
      description: "Keep track of your upcoming and completed tests.",
      accentColor: "#3b8fd9",
      accentBg: "#e8f2fc",
    },
    {
      key: "appointments",
      icon: <Stethoscope className="w-7 h-7" style={{ color: '#e8843c' }} />,
      badgeIcon: <CalendarIcon className="w-3.5 h-3.5" />,
      title: "Appointments",
      description: "Manage upcoming doctor visits and checkups.",
      accentColor: "#e8843c",
      accentBg: "#fdf0e6",
    },
    {
      key: "anc",
      icon: <HeartPulse className="w-7 h-7" style={{ color: '#8b5cf6' }} />,
      badgeIcon: <Activity className="w-3.5 h-3.5" />,
      title: "ANC Visits",
      description: "Track your ANC visits and check your progress.",
      accentColor: "#8b5cf6",
      accentBg: "#f0ecff",
    },
  ];
  ...
  return (
    <div className="min-h-screen py-12 bg-background carelog-page">
      <div className="container max-w-4xl">
        <ScrollReveal>
          ...
          {/* Header */}
          <div className="mb-8">
            <div className="carelog-pill-badge">
              💊 Care Log
            </div>
            <h1 className="carelog-hero-title">
              Care <span className="carelog-hero-gradient">Log</span>
            </h1>
            <p className="mt-3 text-muted-foreground max-w-xl" style={{ fontSize: '0.975rem', lineHeight: '1.65' }}>
              Never miss a dose — smart reminders, snooze options,
              and complete medicine tracking for your pregnancy journey.
            </p>
          </div>
        </ScrollReveal>
        ...
        <div className="space-y-5">
          ...
          {activeTab === "landing" && (
            <>
              <ScrollReveal delay={50}>
                <div className="carelog-card-grid">
                  {dashboardCards.map((card, i) => (
                    <ScrollReveal key={card.key} delay={80 + i * 60}>
                      <CareLogDashboardCard
                        icon={card.icon}
          ...
          {/* Back to dashboard button when in a tab */}
          {activeTab !== "landing" && (
            <ScrollReveal>
              <button
                onClick={() => setActiveTab("landing")}
                className="carelog-back-btn"
              >
                <ArrowLeft className="w-4 h-4" />
                Back to Care Log
              </button>
          ...
          {/* Tab bar — only visible when not on landing */}
          {activeTab !== "landing" && (
            <ScrollReveal delay={50}>
              <div className="flex items-center gap-1 p-1.5 rounded-2xl bg-muted/50 border border-border/40 overflow-x-auto">
                {tabs.map((tab) => {
                  const Icon = tab.icon;
                  return (
                    <button
                      ...
                      className={`flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 whitespace-nowrap ${activeTab === tab.key
                ...
                <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-2">
                  {(Object.keys(FILTER_LABELS) as AppointmentFilter[]).map((filter) => (
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${appointmentFilter === filter
